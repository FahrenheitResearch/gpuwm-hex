...
* the u/w/theta applications (``u_dissipation_3d`` at dissipation-models
  ...
  stencils already mirrored by :mod:`mpas_port.mixing` for v8.2.3, so those
...
def initialize_deformation_weights_v841(
    ...
    *,
    ...
    for cell in range(n_cells):
        ...
        # Halo guard (core lines 1702-1716): the serial global mesh has every
        # neighbor resident, so the mirrored check can only refuse a corrupt
        # connectivity table.
        neighbors = cells_on_cell[cell, :count]
        if np.any((neighbors < 0) | (neighbors >= n_cells)):
            raise ValueError(f"cellsOnCell reaches outside the mesh at cell {cell}")
...
def compute_dry_mixing_tendencies_v841(
    ...
    *,
    ...
    applications reuse the :mod:`mpas_port.mixing` authorities, which are the
