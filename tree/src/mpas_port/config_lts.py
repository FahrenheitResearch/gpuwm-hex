...
``config_v841.py`` is one of the seventeen exact-byte pinned execution sources
(``tools/run_cuda_v841_full_physics_x4.py:199-259``), so these fields live in a
sibling subclass in this unpinned module instead.  The parent's ``validate()``
runs unchanged through a reconstructed parent instance, which keeps every
pinned knob -- ``config_dt=120.0`` included -- exactly as declared.  Local time
stepping does not move the model time step: it changes how many acoustic
sub-steps a column executes inside the step it already had.
"""
