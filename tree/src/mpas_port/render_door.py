...
# MPAS_PORT_* pair came from the import package and predates the decision
# that the brand and the spellings a user types never carry the MPAS
# token.  They are kept AHEAD of nothing and BEHIND the new
...
# the worst possible way to learn about one.
#
# The spellings are stated here and CONSUMED by :mod:`mpas_port.engines`,
...
def convert_frames(convert_exe: Path, *, histories: list[Path], mesh: Path,
                   scratch: Path, window: str, field_set: str,
                   simulation_start: str | None,
                   nc_format: str) -> tuple[dict, list[str]]:
    ...
    command: list[str] = [
        str(convert_exe),
        "--history", *[str(item) for item in histories],
        "--mesh", str(mesh),
...
def run_render(args) -> int:
    ...
    histories = [
        _require_file(
            Path(item), "--history",
            "Give the run's history.*.nc output files; diag files do not "
            "carry the 3-D state the converter reads.")
    ...
    if len(frames_meta) != len(histories):
        raise RenderDoorError(
...
def add_render_arguments(parser) -> None:
    ...
    parser.add_argument(
        ...
        help="MPAS history netCDF file(s), one frame each")
    parser.add_argument(
        "--mesh", required=True, metavar="FILE",
        help="the run's grid/static netCDF with cell coordinates")
    parser.add_argument(
    ...
    parser.add_argument(
        "--window", default="focus", choices=("focus", "global"),
        help="converter render window (default: focus)")
    parser.add_argument(
