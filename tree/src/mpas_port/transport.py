...
def _topology(mesh: Any) -> tuple[int, int, NDArray[np.int64], NDArray[np.int64], NDArray[np.int64], NDArray[np.int64]]:
    ...
    for cell in range(n_cells):
        ...
        if np.any(cells_on_cell[cell, :count] < 0):
            raise ConfigurationRefusal(
                "config_apply_lbcs",
                False,
                "active regional neighbor sentinels need lateral-boundary halo values",
                "config_apply_lbcs=True after supplying the regional halo state",
            )
...
def build_advection_coefficients(
    ...
    *,
    ...
    n_cells, n_edges, counts, _, cells_on_cell, cells_on_edge = _topology(mesh)
    ...
    for edge in range(n_edges):
        ...
        ordered = sorted(unique, key=lambda cell: int(ids[cell]))
        ...
        for neighbor_slot in range(int(counts[cell1])):
        ...
        for neighbor_slot in range(int(counts[cell2])):
...
def initialize_deriv_two(mesh: Any, *, stencil_width: int | None = None) -> FloatArray:
    ...
    n_cells, n_edges, counts, edges_on_cell, cells_on_cell, cells_on_edge = _topology(mesh)
...
def _refuse_lateral_boundaries(config_apply_lbcs: bool) -> None:
    if config_apply_lbcs:
        raise ConfigurationRefusal(
            "config_apply_lbcs",
            config_apply_lbcs,
            "the regional specified/relaxation-zone scalar update needs its driving halo state",
            "config_apply_lbcs=False for a closed mesh",
        )
...
def advance_scalars(
    ...
    *,
    ...
    _refuse_lateral_boundaries(config_apply_lbcs)
    horizontal_order = _validate_order(config_scalar_adv_order, "config_scalar_adv_order")
    vertical_order = _validate_order(config_scalar_vadv_order, "config_scalar_vadv_order")
    _validate_third_order_coefficient(config_coef_3rd_order)
...
def advance_scalars_monotonic(
    ...
    *,
    ...
    for scalar in range(n_tracers):
        ...
        scale_in = np.zeros((n_levels, n_cells), dtype=dtype)
        scale_out = np.zeros((n_levels, n_cells), dtype=dtype)
        for cell in range(n_cells):
            for interface in range(1, n_levels):
                velocity = vertical_velocity[interface, cell]
                flux_upwind = step * (
