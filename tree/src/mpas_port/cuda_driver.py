...
def _v841_physics_cadences(config: DryDycoreConfig) -> dict[str, float]:
    return {
        ...
        "convection": float(config.config_cudt_seconds),
        ...
        if (k > 0 && k < nlev) {
            ...
            if (k == 1 || k == nlev - 1) {
                ...
                value = mpas_add(mpas_div(mpas_mul(velocity, mpas_sub(
                    ...
                    mpas_add(qp1, qm2))), 12.0f), mpas_div(mpas_mul(
                        mpas_abs(velocity), mpas_sub(mpas_sub(qp1, qm2),
                            mpas_mul(3.0f, mpas_sub(qi, qm1)))), 12.0f));
            }
        }
        flux[index] = value;
    }
...
class CudaV841PhysicsStepReceipt(CudaV841StepReceipt):
    ...
    physics_cadences_seconds: dict[str, float]
    gwd_scheme: str
...
def _validate_v841_host_mesh(mesh: Any) -> tuple[int, int, int, int]:
    ...
    if np.any(cells_on_edge < 0) or np.any(cells_on_edge >= n_cells):
        raise ConfigurationRefusal(
            "config_apply_lbcs",
            True,
            "v8.4.1 CUDA requires two in-range cells on every edge",
    ...
    if specified is not None:
        ...
        if np.any(value != 0.0):
            raise ConfigurationRefusal(
                "config_apply_lbcs",
                True,
                "the v8.4.1 CUDA lane is closed/global",
                "a zero specified-zone mask",
            )
...
class CudaDryDycoreDriver:
    ...
    def _kernel(self, name: str) -> Any:
        ...
        if result is None:
            result = self.cache.raw_kernel(
                name,
                _CUDA_SOURCE,
                module_key="mpas_port.cuda_driver",
            )
    ...
    def _physics_driver_kernel(self, name: str) -> Any:
        ...
        if result is None:
            result = self.cache.raw_kernel(
                ...
                module_key="mpas_port.cuda_driver.physics_v841",
            )
    ...
    def _advance_dynamics_subcycle_v841(
        ...
        *,
        ...
        if (physics_tendencies is None) != (moist_coefficients is None):
            raise ValueError(
