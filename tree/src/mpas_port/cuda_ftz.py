...
PERFORMANCE_RATIO_CEILING = 1.25
...
def validate_compile_manifest_relation(
    ...
    if fingerprint.get("device_compute_capability") != "120":
        raise FtzContractError(
            "MPAS FTZ contract requires device_compute_capability=120"
        )
...
def validate_v841_compile_manifest_relation(
    ...
    if fingerprint.get("device_compute_capability") != "120":
        raise FtzContractError(
            "MPAS v8.4.1 FTZ contract requires device_compute_capability=120"
        )
...
def build_mpas_ftz_binding(
    *,
    ...
    audit_module_labels = {
        "mpas_port.cuda_acoustic": "acoustic",
        "mpas_port.cuda_backend.recovery": "recovery",
        "mpas_port.cuda_driver": "driver",
        "mpas_port.cuda_horizontal": "horizontal",
    }
    ...
    if not all(
        ...
        and float(row.get("enabled_over_disabled", 99.0)) <= PERFORMANCE_RATIO_CEILING
    ...
    return {
        ...
        "claim": (
            ...
            "measured terminal -ftz=true route for which gpuwm's sm_120 probe "
...
def _validate_v841_kernel_audit_structure(
    ...
    *,
    ...
    if (
        ...
        or audit.get("measurement") != V841_KERNEL_AUDIT_MEASUREMENT
    ...
    *,
    ...
    return {
        ...
        "claim": (
            ...
            "under the measured terminal -ftz=true sm_120 route. Every one of "
...
def run_scalar_transport_subnormal_deck() -> dict[str, Any]:
    ...
    return {
        ...
        "fallback_patch_map": [
            {
                "translation_unit": "mpas_port.cuda_transport",
                "kernels": [
                    "transport_edge_values",
                    "transport_vertical_flux",
                    "transport_target_density",
...
def _run_guarded_kernel_audit_once(
    *,
    ...
    def kernel(module: str, name: str) -> Any:
        ...
        module_key = (
            f"mpas_port.ftz_audit.{module}.{mode}"
            if transcript_module_keys is None
...
def run_normalized_fallback_performance_control(
    *,
    ...
    declared 1.25x ceiling.  The passes are interleaved to reduce clock drift.
    """
    ...
    def raw(module: str, name: str, mode: str) -> Any:
        ...
        return caches[mode][module].raw_kernel(
            ...
            module_key=f"mpas_port.ftz_perf.{module}.{mode}",
        )
    ...
    for module, name, total, builder in benchmarks:
        ...
        results[key] = {
            ...
            "ceiling": PERFORMANCE_RATIO_CEILING,
        ...
        if ratio > PERFORMANCE_RATIO_CEILING:
            raise FtzContractError(
                f"normalized fallback median regressed at {key}: "
                f"{ratio:.6f}x > {PERFORMANCE_RATIO_CEILING:.2f}x"
            )

    return {
        "schema": PERFORMANCE_CONTROL_SCHEMA,
        "declared_median_ratio_ceiling": PERFORMANCE_RATIO_CEILING,
...
def v841_reached_translation_units() -> dict[str, tuple[str, tuple[str, ...]]]:
    ...
    particular, ``mpas_port.cuda_transport`` and the FCT-only exports in the
    ...
    sources = {
        "mpas_port.cuda_acoustic": cuda_acoustic._CUDA_SOURCE,
        "mpas_port.cuda_acoustic_v841": cuda_acoustic_v841._CUDA_SOURCE,
        "mpas_port.cuda_backend.recovery": RECOVERY_CUDA_SOURCE,
        "mpas_port.cuda_driver": cuda_driver._CUDA_SOURCE,
        "mpas_port.cuda_dynamics_v841": cuda_dynamics_v841._CUDA_SOURCE,
        "mpas_port.cuda_horizontal": cuda_horizontal._CUDA_SOURCE,
    ...
    kernels = {
        "mpas_port.cuda_acoustic": ("tendency_w_to_omega",),
        "mpas_port.cuda_acoustic_v841": (
...
def production_translation_units() -> dict[str, tuple[str, tuple[str, ...]]]:
    ...
    sources = {
        ...
        "mpas_port.cuda_driver": cuda_driver._CUDA_SOURCE,
        "mpas_port.cuda_horizontal": cuda_horizontal._CUDA_SOURCE,
        "mpas_port.cuda_transport": cuda_transport._CUDA_SOURCE,
    }
    result: dict[str, tuple[str, tuple[str, ...]]] = {}
    direct = re.compile(
        r'extern\s+"C"\s+__global__\s+void\s+([A-Za-z_][A-Za-z0-9_]*)\s*\('
