...
# ---------------------------------------------------------------------------
# device admission (sm_86 lives here; the frozen runner keeps its own constants)
# ---------------------------------------------------------------------------
...
#: Measured resident whole-mesh footprint of the x4.163842 full-physics case
#: (brief-1 measurement, restated at ``build_partition_assets_v841.py``).
WHOLE_MESH_RESIDENT_BYTES_X4 = int(22.0 * _GIB)
...
def partition_min_free_bytes(
    ...
    n_global_cells: int,
    *,
    whole_mesh_resident_bytes: int = WHOLE_MESH_RESIDENT_BYTES_X4,
    headroom: float = 1.25,
) -> int:
    """The admission floor for ONE partition's device.

    Gate law (multi-GPU design D2): the free-memory floor must reflect the
    actual partition need, not the whole-mesh constant.  The 20 GiB default
    below was written for whole-mesh work and REFUSES a 16 GB card even
    though the small side of the 2:1 split needs only ~7.6 GiB resident; the
    two-node runner therefore passes this scaled floor explicitly.  Residency
    scales with local (owned + halo) cell count, measured at 22.0 GiB for the
    163,842-cell whole mesh; ``headroom`` covers workspace churn.
    """
    ...
    scaled = whole_mesh_resident_bytes * (float(n_local_cells) / float(n_global_cells))
    return int(scaled * float(headroom))
...
def require_devices(
    ...
    *,
    ...
    min_free_bytes: int = 20 * _GIB,
    cupy_module: Any = None,
) -> list[dict[str, Any]]:
    """Admission for the partitioned tools.

    Carries its own thresholds -- the frozen runner's ``(12, 0)`` / 24 GiB
    constants are untouched.  ``min_free_bytes`` defaults to the whole-mesh
    working floor; partitioned callers MUST pass the
    :func:`partition_min_free_bytes` scaled floor instead (the default refuses
    a 16 GB card whose partition share is a fraction of it).  Returns a
    per-device receipt.
    """
