...
def advance_acoustic_step_v841(
    ...
    *,
    ...
    if specified_zone_edge is not None and np.any(np.asarray(specified_zone_edge) != 0.0):
        raise ConfigurationRefusal(
            "config_apply_lbcs", True, "the v8.4.1 CPU lane is closed/global", "config_apply_lbcs=False"
        )
    if specified_zone_cell is not None and np.any(np.asarray(specified_zone_cell) != 0.0):
        raise ConfigurationRefusal(
            "config_apply_lbcs", True, "the v8.4.1 CPU lane is closed/global", "config_apply_lbcs=False"
        )
