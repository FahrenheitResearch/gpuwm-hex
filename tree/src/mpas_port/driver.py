    ...
    def validate(self) -> None:
        ...
        if (
            ...
            _refuse(
                ...
                "native split-three Smagorinsky has no packaged compiled-MPAS oracle yet",
                "config_horiz_mixing='off' or zero-coefficient '2d_fixed'",
            )
        unsupported_bools = (
...
def _pressure_exner(
    ...
    *,
    ...
    exner = argument ** dtype.type(rgas / (cp - rgas))
    ...
    *,
    ...
    with Dataset(state_path, mode="r") as dataset:
        ...
        if "theta_m" in dataset.variables:
            ...
            theta_m = theta_dry * (
                rho.dtype.type(1.0)
                + rho.dtype.type(water_vapor_rgas / rgas) * qv
            )
        else:
        ...
        cells = _mesh_array(mesh, "cellsOnEdge").astype(np.int64, copy=False)
        rho_edge = rho.dtype.type(0.5) * (rho[:, cells[:, 0]] + rho[:, cells[:, 1]])
        ...
        if "rw" in dataset.variables:
            ...
            rho_w = interface_density * interface_zz * w
        ...
        *,
        ...
        if np.any(cells < 0):
            _refuse(
                ...
                "config_apply_lbcs=False on a closed global mesh",
            )
        has_terrain = np.max(np.abs(vertical_grid.zxu), initial=0.0) > 2.0e-14
        ...
        *,
        ...
        if self.mixing_config is not None:
            saved_mixing = compute_dry_mixing_tendencies(
                ...
                # Frozen F:810 passes the outer transport timestep into every
                # dynamics subcycle; the Smagorinsky cap at F:4641/4670 must
                # therefore never see the shorter dynamics timestep.
                dt=outer_dt,
        ...
        for stage in schedule.stages:
            ...
            current_normal_velocity = _normal_velocity(current, self.mesh)
            current_vertical_velocity = _interface_velocity(
                current,
                self.vertical_grid,
                self.mesh,
                self.terrain_metrics,
