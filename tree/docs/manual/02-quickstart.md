...
The declaration now carries a bounded range — `gpuwm>=2.6.1,<2.6.2` — so pip
...
- **2.6.1 is the only published engine that matches**, re-measured 2026-09-01
...
- **A newer gpuwm will not be resolved either.** The ceiling sits at the
  ...
  2.6.2 is excluded exactly as 2.6.0 is. That is deliberate: an unmeasured
...
- **If you already have a different gpuwm in the environment**, `gpuwm-hex
  ...
  reads `gpuwm 2.6.1: 16 of 16 pinned files are in this install and all 16
  match` (measured 2026-09-01 against the published 2.6.1 wheel, in a fresh
...
this distribution's declared range is `gpuwm>=2.6.1,<2.6.2` (the floor is
...
not in the wheel) and `--gpuwm-checkout` is a gpuwm checkout at **`v2.6.1`**,
...
gpuwm-hex forecast --preflight \
  ...
  --gpuwm-checkout <gpuwm-checkout-at-v2.6.1>
```
